...
file_open = open(f'/home/masson/afterglowpy/data/orphans_pseudo_obs_5pts_low_redshift.pkl', 'rb')
...
for config in df_orphans['config']:
    i += 1
    if config['thetaWing'] < config['thetaCore']:
        all_i.append(i)

df_orphans = df_orphans.drop(all_i)
...
def clean_and_sort_light_curve(grb_time, time, mags, mags_err, mags_lim, filt):
    """
    ...
    mags_lim: pandas.Series of list of float
        Concatenated limiting magnitude for the object
    ...
    # convert colors in filters
    filtercolors = {'b': 'u', 'c': 'g', 'g': 'r', 'orange': 'i', 'r': 'z', 'm': 'Y'}

    for i, t, m, e, l, f in zip(grb_time, time, mags, mags_err, mags_lim, filt):
        # Convert to numpy arrays
        t = np.array(t)
        m = np.array(m)
        e = np.array(e)
        l = np.array(l)
        f = np.array([filtercolors[one_f] for one_f in f])
    ...
    return pd.Series(cleaned_grb_time), pd.Series(cleaned_times), pd.Series(cleaned_magnitudes), pd.Series(cleaned_errors), pd.Series(
        cleaned_filters)
...
def compute_colours(times, mags, filts):
    ...
    #filter_pairs = [('b', 'c'), ('c', 'g'), ('g', 'orange'), ('orange', 'r'), ('r', 'm')]
...
print('Cleaning data...')
grb_time, times, mags, err, filts = clean_and_sort_light_curve(df_orphans['grb_time'],
                                                               df_orphans['time'],
                                                               df_orphans['mags_var'],
                                                               df_orphans['mags_err'],
                                                               df_orphans['mags_lim'],
...
duration = np.array(
    [
        compute_duration_between_first_and_peak(i, j) for i, j in zip(times.values, mags.values)
    ]
)
...
rates = np.array(
    [
        compute_rates(i, j, k) for i, j, k in zip(times.values, mags.values, filts.values)
    ]
)
...
colours = np.array(
    [
        compute_colours(i, j, k) for i, j, k in zip(times.values, mags.values, filts.values)
    ]
)

print('Fitting...')
# parameters fit A, B, C, D and chi1
fit_parameters = np.array(
    [
        fit_light_curve(i, j, k, f) for i, j, k, f in zip(times.values, tqdm(mags.values), err.values, filts.values)
    ]
)
...
file = open(f'/home/masson/afterglowpy/data/orphans_features.pkl', 'wb')
