...
file_open = open(f'../data/elasticc/data_no_periodic.pkl', 'rb')
df_elasticc = pickle.load(file_open)
file_open.close()

df_elasticc_sample = df_elasticc.sample(frac=1)
...
def clean_and_sort_light_curve(alert_id, time, mags, mags_err, filt):
    """
    ...
    for i, t, m, e, f in zip(alert_id, time, mags, mags_err, filt):
    ...
    return pd.Series(cleaned_alert_id), pd.Series(cleaned_times), pd.Series(cleaned_magnitudes), pd.Series(cleaned_errors), pd.Series(
        cleaned_filters)
...
print('Cleaning data...')
alert_id, times, mags, err, filts = clean_and_sort_light_curve(df_elasticc_sample['alertId'],
                                                               df_elasticc_sample['time'],
                                                               df_elasticc_sample['mags'],
                                                               df_elasticc_sample['mags_err'],
                                                               df_elasticc_sample['filt'])
...
file = open(f'../data/elasticc/elasticc_new_features_D_not_constrained.pkl', 'wb')
